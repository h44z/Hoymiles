...
namespace Hoymiles
{
    ...
    {
        memset(_payloadDevInfoAll, 0, HM_DEV_INFO_SIZE);
        _devInfoAllLength = 0;
    }

    void DevInfoParser::appendFragmentAll(uint8_t offset, uint8_t* payload, uint8_t len)
    {
        if (offset + len > HM_DEV_INFO_SIZE) {
            PERRF("FATAL: (%s, %d) dev info all packet too large for buffer\n", __FILE__, __LINE__);
            return;
        }
    ...
    {
        memset(_payloadDevInfoSample, 0, HM_DEV_INFO_SIZE);
    ...
    {
        if (offset + len > HM_DEV_INFO_SIZE) {
            PERRF("FATAL: (%s, %d) dev info Sample packet too large for buffer\n", __FILE__, __LINE__);
            return;
        }
