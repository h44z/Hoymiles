...
{
    memset(_payloadAlarmLog, 0, HM_ALARM_LOG_ENTRY_COUNT * HM_ALARM_LOG_ENTRY_SIZE);
    _alarmLogLength = 0;
}

void AlarmLogParser::appendFragment(uint8_t offset, uint8_t* payload, uint8_t len)
{
    if (offset + len > (HM_ALARM_LOG_ENTRY_COUNT * HM_ALARM_LOG_ENTRY_SIZE)) {
...
{
    return (_alarmLogLength - 2) / HM_ALARM_LOG_ENTRY_SIZE;
}

void AlarmLogParser::getLogEntry(uint8_t entryId, AlarmLogEntry_t* entry)
{
    uint8_t entryStartOffset = 2 + entryId * HM_ALARM_LOG_ENTRY_SIZE;
