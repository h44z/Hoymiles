...
bool MultiDataCommand::handleResponse(InverterAbstract* inverter, hm_fragment_t fragment[], uint8_t max_fragment_id)
{
    // All fragments are available --> Check CRC
    uint16_t crc = 0xffff, crcRcv = 0x0;

    for (uint8_t i = 0; i < max_fragment_id; i++) {
        if (i == max_fragment_id - 1) {
            // Last packet
            crc = hm_crc16(fragment[i].fragment, fragment[i].len - 2, crc);
            crcRcv = (fragment[i].fragment[fragment[i].len - 2] << 8)
                | (fragment[i].fragment[fragment[i].len - 1]);
        } else {
            crc = hm_crc16(fragment[i].fragment, fragment[i].len, crc);
        }
    }
...
{
    uint16_t crc = hm_crc16(&_payload[10], 14); // From data_type till password
